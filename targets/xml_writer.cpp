...
void mml::xml_writer::do_stop_node(mml::stop_node * const node, int lvl) {
  //ASSERT_SAFE_EXPRESSIONS;
  openTag(node, lvl);
  os() << " svalue='" << node->svalue() << std::endl;
  closeTag(node, lvl);
}
...
void mml::xml_writer::do_next_node(mml::next_node * const node, int lvl) {
  //ASSERT_SAFE_EXPRESSIONS;
  openTag(node, lvl);
  os() << " nvalue='" << node->nvalue() << std::endl;
  closeTag(node, lvl);
}
...
static std::string get_qualifier(int qualifier) {
  ...
  if (qualifier == tFOREIGN) return "external built-in";
  if (qualifier == tFORWARD) return "external user";
...
void mml::xml_writer::do_variable_declaration_node(mml::variable_declaration_node * const node, int lvl) {
  //ASSERT_SAFE_EXPRESSIONS;
  os() << std::string(lvl, ' ') << "<" << node->label() << " name='" << node->identifier() << "' qualifier='" << get_qualifier(node->qualifier()) << "' type='" << cdk::to_string(node->type()) << "'>" << std::endl;
  if (node->initializer()) {
